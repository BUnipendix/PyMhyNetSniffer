...
class Sniffer:
    def __init__(self, pipe_name='genshin_packet_pipe', dump_file: str = None, cache_packet=True,
    ...
    def add_to_list(self, packet_name, list_name: str):
        """
        :param packet_name: Packet name like 'PingRsp'
        :param list_name: "whitelist" or 'blacklist'
        """
        if list_name.lower() == 'whitelist':
            self.whitelist.append(packet_name)
        elif list_name.lower() == 'blacklist':
            self.blacklist.append(packet_name)
    ...
    def _packet_process_loop(self, override_func=None):
        ...
        while True:
            ...
            if packet.packet_name in self.blacklist:
                del raw_packet
                continue
            if len(self.whitelist) > 0 and packet.packet_name not in self.whitelist:
                del raw_packet
                continue
